...
class AvenueDataset(torch.utils.data.Dataset):
    ...
    def _read_data(self):
        ...
        for teacher in self.config['teachers']:
            train_data_path = self.config['av_train_data_path']
            train_labels_path = self.config[f'av_train_labels_path_{teacher}']
            test_data_path = self.config['av_test_data_path']
            test_labels_path = self.config[f'av_test_labels_path_{teacher}']
            test_labels_true_path = self.config['av_test_labels_true_path']
            ...
            if self.train is True:
                ...
                for path in glob.glob(os.path.join(test_data_path, "*")):
                    ...
                    # Extract true labels
                    lbls = np.loadtxt(os.path.join(test_labels_true_path, f"{path.split('/')[-1][:2]}.txt"))
    ...
    def __getitem__(self, index):
        ...
        if self.train is True:
            ...
            current_img = np.concatenate([previous_img, current_img, next_img], axis=-1)

            current_img = (current_img - 127.) / 128.

            current_img = np.swapaxes(current_img, 0, -1)

            # Uncomment for 3D input
            return current_img, label, true_label, \
                   int(self.data[index].split('/')[-2]), \
